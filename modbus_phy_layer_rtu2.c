...
extern int32 debug_stamp[2];
extern int32 STAMP100us;
char ttest[16];
int count_test ;
...
//int1 test;
int1 modbus_timed_out = 0;
int1 modbus_timeout_request = 0;
...
//   #if (MODBUS_TIMER_UPDATE == MODBUS_TIMER_NOISR)
   ...
   //rate, and the get_ticks(MODBUS) below will not always be called
...
//get_ticks(MODBUS);
  // #endif
   ...
   //an old timer value is used in comparison
	if(modbus_timeout_enabled && (modbus_timed_out))
	{
...
void modbus_serial_send_start(unsigned int8 to, unsigned int8 func)
{
   ...
   output_high(MODBUS_SERIAL_ENABLE_PIN);  //TX enable
#endif
...
void incomming_modbus_serial() {
   char c;
   	...
   	c=fgetc(MODBUS_SERIAL);						// get 8 bits
 	ttest[count_test] = c;
 	if (count_test<16)
	count_test++;
	else 
   ...
   if (!modbus_serial_new)						// if previous completed not waiting
   {
      if(modbus_serial_state == MODBUS_GETADDY)	// first 8 bits to the address
      {
         modbus_serial_crc.d = 0xFFFF;
         modbus_rx.address = c;
         modbus_serial_state++;					
      ...
      else if(modbus_serial_state == MODBUS_GETFUNC) // second 8 bits to the function	
      {
         modbus_rx.func = c;
         modbus_serial_state++;
      }
      else if(modbus_serial_state == MODBUS_GETDATA) // rest goes to offset + data
      {
         if (modbus_rx.len>=MODBUS_SERIAL_RX_BUFFER_SIZE)		// Buffer stores one entry
       {
      ...
      modbus_timeout_enabled = (TRUE); 
	  modbus_timed_out = 0;				
	  modbus_timeout_request = 1;		// request will reset timer and request flag
	
      modbus_calc_crc(c);
	  debug_stamp[0]  = STAMP100us;
		// MODBUS_kbhit will handle the shit....	
   }
