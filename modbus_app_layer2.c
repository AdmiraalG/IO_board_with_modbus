...
int1 modbus_kbhit()
{
   ...
   modbus_check_timeout(); //checks for packet integrity and sets modbus_serial_new if all ok
   #endif
                                        ...
                                        unsigned int16 *reg_data)
{
   ...
   {
      modbus_serial_putc(make8(*reg_data,1));//MSB FIRST
      modbus_serial_putc(make8(*reg_data,0));//LSB SECOND
      reg_data++;
   }
